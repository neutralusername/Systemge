...
func (server *Server) oauth2AuthCallback() http.HandlerFunc {
	return func(responseWriter http.ResponseWriter, httpRequest *http.Request) {
		if infoLogger := server.infoLogger; infoLogger != nil {
			infoLogger.Log(Event.New("Oauth2 callback for \""+httpRequest.RemoteAddr+"\" called", nil).Error())
		}
		...
		if state != server.config.Oauth2State {
			if warningLogger := server.warningLogger; warningLogger != nil {
				warningLogger.Log(Event.New("Failed oauth2 state check for \""+state+"\" for client \""+httpRequest.RemoteAddr+"\"", nil).Error())
			}
		...
		if err != nil {
			if warningLogger := server.warningLogger; warningLogger != nil {
				warningLogger.Log(Event.New("Failed exchanging code \""+code+"\" for token for client \""+httpRequest.RemoteAddr+"\"", err).Error())
			}
			http.Redirect(responseWriter, httpRequest, server.config.CallbackFailureRedirectUrl, http.StatusMovedPermanently)
			return
		}
		...
		if session == nil {
			if warningLogger := server.warningLogger; warningLogger != nil {
				warningLogger.Log(Event.New("Failed creating session for access token \""+token.AccessToken+"\" for client \""+httpRequest.RemoteAddr+"\"", nil).Error())
			}
			http.Redirect(responseWriter, httpRequest, server.config.CallbackFailureRedirectUrl, http.StatusMovedPermanently)
			return
		}
		if infoLogger := server.infoLogger; infoLogger != nil {
			infoLogger.Log(Event.New("Created session for access token \""+token.AccessToken+"\" for client \""+httpRequest.RemoteAddr+"\" with sessionId \""+session.sessionId+"\" and identity \""+session.identity+"\"", nil).Error())
		}
...
func (server *Server) oauth2Auth() http.HandlerFunc {
	return func(responseWriter http.ResponseWriter, httpRequest *http.Request) {
		...
		if infoLogger := server.infoLogger; infoLogger != nil {
			infoLogger.Log(Event.New("Oauth2 auth request by \""+httpRequest.RemoteAddr+"\"", nil).Error())
		}
