...
func MultiAsyncMessage(topic, payload string, connections ...*TcpConnection) <-chan error {
...
func MultiSyncRequest(topic, payload string, connections ...*TcpConnection) (<-chan *Message.Message, <-chan error) {
	...
	for _, connection := range connections {
		...
		if err != nil {
			...
			waitGroup.Add(1)
			go func(connection *TcpConnection, responseChannel <-chan *Message.Message) {
