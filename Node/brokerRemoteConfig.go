...
func (node *Node) AddSyncTopicRemotely(brokerConfigEndpoint TcpEndpoint.TcpEndpoint, topics ...string) error {
	...
	defer netConn.Close()
	_, err = Utilities.TcpExchange(netConn, Message.NewAsync("addSyncTopics", node.GetName(), payload), node.config.TcpTimeoutMs)
	if err != nil {
...
func (node *Node) RemoveSyncTopicRemotely(brokerConfigEndpoint TcpEndpoint.TcpEndpoint, topics ...string) error {
	...
	defer netConn.Close()
	_, err = Utilities.TcpExchange(netConn, Message.NewAsync("removeSyncTopics", node.GetName(), payload), node.config.TcpTimeoutMs)
	if err != nil {
		return Error.New("failed exchanging messages with broker", err)
	}
	return nil
...
func (node *Node) AddAsyncTopicRemotely(brokerConfigEndpoint TcpEndpoint.TcpEndpoint, topics ...string) error {
	...
	defer netConn.Close()
	_, err = Utilities.TcpExchange(netConn, Message.NewAsync("addAsyncTopics", node.GetName(), payload), node.config.TcpTimeoutMs)
	if err != nil {
		return Error.New("failed exchanging messages with broker", err)
	}
	return nil
...
func (node *Node) RemoveAsyncTopicRemotely(brokerConfigEndpoint TcpEndpoint.TcpEndpoint, topics ...string) error {
	...
	defer netConn.Close()
	_, err = Utilities.TcpExchange(netConn, Message.NewAsync("removeAsyncTopics", node.GetName(), payload), node.config.TcpTimeoutMs)
	if err != nil {
		return Error.New("failed exchanging messages with broker", err)
	}
	return nil
