...
func (node *Node) getBrokerConnectionForTopic(topic string) *brokerConnection {
	...
	if brokerConnection == nil {
		...
		node.GetLogger().Info(Error.New("Resolved broker address \""+endpoint.GetAddress()+"\" for topic \""+topic+" \" on node \""+node.GetName()+"\"", nil).Error())
		brokerConnection = node.getBrokerConnection(endpoint.GetAddress())
		if brokerConnection == nil {
			brokerConnection, err = node.connectToBroker(endpoint)
			if err != nil {
				node.GetLogger().Warning(Error.New("Failed connecting to broker \""+endpoint.GetAddress()+"\" for topic \""+topic+"\" on node \""+node.GetName()+"\"", err).Error())
				return nil
			}
			...
			if err != nil {
				...
				node.GetLogger().Warning(Error.New("Failed adding broker connection \""+endpoint.GetAddress()+"\" for topic \""+topic+"\" on node \""+node.GetName()+"\"", err).Error())
				return nil
			}
		...
		if err != nil {
			...
			node.GetLogger().Warning(Error.New("Failed adding topic resolution for topic \""+topic+"\" to broker connection \""+brokerConnection.endpoint.GetAddress()+"\" on node \""+node.GetName()+"\"", err).Error())
			return nil
		}
		node.GetLogger().Info(Error.New("Added topic resolution for topic \""+topic+"\" to broker connection \""+brokerConnection.endpoint.GetAddress()+"\" on node \""+node.GetName()+"\"", nil).Error())
	} else {
		node.GetLogger().Info(Error.New("Found existing topic resolution \""+brokerConnection.endpoint.GetAddress()+"\" for topic \""+topic+"\" on node \""+node.GetName()+"\"", nil).Error())
	}
