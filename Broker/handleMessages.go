	...
	for broker.isStarted {
		...
		if err != nil {
			if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
				warningLogger.Log(Error.New("Failed to receive message from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
			}
			return
		}
		if infoLogger := broker.node.GetInfoLogger(); infoLogger != nil {
			infoLogger.Log(Error.New("Received message with topic \""+message.GetTopic()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", nil).Error())
		}
		err = broker.validateMessage(message)
		if err != nil {
			if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
				warningLogger.Log(Error.New("Invalid message with topic \""+message.GetTopic()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
			}
			return
		}
		if message.GetSyncResponseToken() != "" {
			...
			if err != nil {
				if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
					warningLogger.Log(Error.New("Failed to handle sync response with topic \""+message.GetTopic()+"\" and token \""+message.GetSyncResponseToken()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
				}
			} else {
				if infoLogger := broker.node.GetInfoLogger(); infoLogger != nil {
					infoLogger.Log(Error.New("Received sync response with topic \""+message.GetTopic()+"\" and token \""+message.GetSyncResponseToken()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", nil).Error())
				}
			}
			continue
		}
		...
		if err != nil {
			if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
				warningLogger.Log(Error.New("Invalid topic for message with topic \""+message.GetTopic()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
			}
			return
		}
		if message.GetSyncRequestToken() != "" {
			if err := broker.addSyncRequest(nodeConnection, message); err != nil {
				if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
					warningLogger.Log(Error.New("Failed to add sync request with topic \""+message.GetTopic()+"\" and token \""+message.GetSyncRequestToken()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
				}
				...
				if err != nil {
					if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
						warningLogger.Log(Error.New("Failed to send error response for failed sync request with topic \""+message.GetTopic()+"\" and token \""+message.GetSyncRequestToken()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
					}
				}
				continue
			}
			if infoLogger := broker.node.GetInfoLogger(); infoLogger != nil {
				infoLogger.Log(Error.New("Added sync request with topic \""+message.GetTopic()+"\" and token \""+message.GetSyncRequestToken()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", nil).Error())
			}
		}
		err = broker.handleMessage(nodeConnection, message)
		if err != nil {
			if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
				warningLogger.Log(Error.New("Failed to handle message with topic \""+message.GetTopic()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
			}
		} else {
			if infoLogger := broker.node.GetInfoLogger(); infoLogger != nil {
				infoLogger.Log(Error.New("Handled message with topic \""+message.GetTopic()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", nil).Error())
			}
		}
	}
}
...
func (broker *Broker) handleSubscribe(nodeConnection *nodeConnection, message *Message.Message) error {
	...
	if err != nil {
		...
		if errResponse != nil {
			if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
				warningLogger.Log(Error.New("Failed to send error response for failed subscribe request for topic \""+message.GetTopic()+"\" with token \""+message.GetSyncRequestToken()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", errResponse).Error())
			}
		}
	...
	if err != nil {
		if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
			warningLogger.Log(Error.New("Failed to send sync response for successful subscribe request for topic \""+message.GetTopic()+"\" with token \""+message.GetSyncRequestToken()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
		}
	}
	return nil
}
...
func (broker *Broker) handleUnsubscribe(nodeConnection *nodeConnection, message *Message.Message) error {
	...
	if err != nil {
		...
		if errResponse != nil {
			if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
				warningLogger.Log(Error.New("Failed to send error response for failed unsubscribe request for topic \""+message.GetTopic()+"\" with token \""+message.GetSyncRequestToken()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", errResponse).Error())
			}
		}
	...
	if err != nil {
		if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
			warningLogger.Log(Error.New("Failed to send sync response for successful unsubscribe request for topic \""+message.GetTopic()+"\" with token \""+message.GetSyncRequestToken()+"\" from node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
		}
	}
	return nil
}
...
func (broker *Broker) propagateMessage(message *Message.Message) {
	...
	for _, nodeConnection := range broker.nodeSubscriptions[message.GetTopic()] {
		go func() {
			...
			if err != nil {
				if warningLogger := broker.node.GetWarningLogger(); warningLogger != nil {
					warningLogger.Log(Error.New("Failed to send message with topic \""+message.GetTopic()+"\" to node \""+nodeConnection.name+"\" on broker \""+broker.node.GetName()+"\"", err).Error())
				}
