...
func NewClienn(name string, netConn net.Conn) *ClientConnection {
...
func (client *ClientConnection) resetWatchdog() error {
	client.mutex.Lock()
...
func (client *ClientConnection) disconnect() error {
	client.mutex.Lock()
	defer client.mutex.Unlock()
	if client.watchdog == nil {
		return Error.New("Watchdog is not set for client \""+client.name+"\"", nil)
...
func (server *Server) addClient(client *ClientConnection) error {
	server.mutex.Lock()
	...
	client.watchdog = time.AfterFunc(WATCHDOG_TIMEOUT, func() {
		server.logger.Log("Client \"" + client.name + "\" has timed out")
		server.removeClient(client)
	})
	return nil
}

func (server *Server) removeClient(client *ClientConnection) error {
	server.mutex.Lock()
	defer server.mutex.Unlock()
