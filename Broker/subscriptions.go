...
func (server *Server) addSubscription(client *ClientConnection, topic string) error {
	server.mutex.Lock()
	...
	if server.subscriptions[topic] == nil {
		server.subscriptions[topic] = map[string]*ClientConnection{}
	}
...
func (server *Server) removeSubscription(client *ClientConnection, topic string) error {
	server.mutex.Lock()
	defer server.mutex.Unlock()
	if server.subscriptions[topic] == nil {
