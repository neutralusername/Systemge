...
func (broker *Broker) addResolverTopicsRemotely(topics ...string) error {
	...
	response, _, err := Tcp.Exchange(netConn, Message.NewAsync("addTopics", broker.node.GetName(), payload).Serialize(), broker.config.TcpTimeoutMs, broker.config.IncomingMessageByteLimit)
	if err != nil {
...
func (broker *Broker) removeResolverTopicsRemotely(topics ...string) error {
	...
	response, _, err := Tcp.Exchange(netConn, Message.NewAsync("removeTopics", broker.node.GetName(), payload).Serialize(), broker.config.TcpTimeoutMs, broker.config.IncomingMessageByteLimit)
	if err != nil {
		return Error.New("failed exchanging messages with resolver", err)
	}
	if response.GetTopic() == "error" {
