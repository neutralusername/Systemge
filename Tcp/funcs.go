...
func Exchange(netConn net.Conn, messageBytes []byte, timeoutMs uint64, byteLimit uint64) (*Message.Message, uint64, error) {
	err := Send(netConn, messageBytes, timeoutMs)
	if err != nil {
		return nil, 0, Error.New("Error sending message", err)
	}
	responseBytes, bytesReceived, err := Receive(netConn, timeoutMs, byteLimit)
	if err != nil {
		return nil, 0, Error.New("Error receiving response", err)
	}
	responseMessage := Message.Deserialize(responseBytes)
	if responseMessage == nil {
		return nil, 0, Error.New("Error deserializing response", nil)
	}
	return responseMessage, bytesReceived, nil
}
