...
func (client *TcpConnection) StartReadRoutine(maxConcurrentHandlers uint32, delayNs int64, timeoutNs int64, readHandler Tools.ReadHandler[[]byte, *TcpConnection]) error {
	...
	client.readRoutine = Tools.NewRoutine(
		func(<-chan struct{}) {
			if bytes, err := client.Read(timeoutNs); err == nil {
...
func (client *TcpConnection) StopReadRoutine(abortOngoingCalls bool) error {
	client.readMutex.Lock()
	defer client.readMutex.Unlock()
	...
	err := client.readRoutine.StopRoutine(abortOngoingCalls)
