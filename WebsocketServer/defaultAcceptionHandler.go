...
func (server *WebsocketServer) GetDefaultAcceptionHandler() func(*WebsocketClient.WebsocketClient) error {
	return func(websocketClient *WebsocketClient.WebsocketClient) error {
		...
		session := server.createSession("", websocketClient)
		if session == nil {
		...
		go server.GetDefaultReceptionHandler(session.GetIdentity(), session.GetId())(websocketClient)
